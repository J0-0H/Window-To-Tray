...
namespace I18N {
    ...
    static const wchar_t* zh(const char* key) {
        ...
        if (std::strcmp(key, "settings_use_collection_mode") == 0) return L"启用聚合收纳模式 (左键单击托盘图标时)";
        if (std::strcmp(key, "settings_hotkey_show_collection") == 0) return L"快捷键：显示收纳窗口";
        if (std::strcmp(key, "collection_window_title") == 0) return L"已收纳的窗口";
        // --- NEW ---
        if (std::strcmp(key, "collection_disable_mode_button") == 0) return L"退出收纳模式";
